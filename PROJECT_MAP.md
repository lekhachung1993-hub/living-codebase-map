...
| Current Branch | `main` |
| Codebase-MD5 | `9ad86e94867aea68a2a12f6ff042d455` |
...
| L1836 | `build_symbol_database()` | Full codebase symbol scanner and lookup table builder |
| L1875 | `update_map_line_numbers()` | Synchronizes line numbers in MODULE 1 & 2 tables |
| L1930 | `update_map_header()` | Refreshes date, commit hash, and MD5 in header |
| L1963 | `inject_feature()` | Adds new entry into MODULE 5 Feature Traceability Matrix |
| L1995 | `inject_constraint()` | Appends new operational rule to MODULE 4 |
| L2031 | `cmd_init()` | CLI handler for `living-map init` |
| L2098 | `cmd_update()` | CLI handler for `living-map update` |
| L2169 | `cmd_check()` | CLI handler for CI/CD smart drift check |
| L2243 | `cmd_impact()` | CLI handler for blast radius impact analysis |
| L2399 | `cmd_deep_impact()` | CLI handler for 6-layer architecture dependency tree |
| L2403 | `cmd_install_hook()` | CLI handler for installing Git pre-commit / pre-push hooks |
| L2451 | `cmd_add_feature()` | CLI handler for feature registration (supports NLP auto-parsing) |
| L2576 | `cmd_add_constraint()` | CLI handler for adding constraints to Module 4 |
| L2639 | `cmd_plan()` | CLI handler for graph-backed change planning |
| L2664 | `cmd_verify_change()` | CLI handler for diff/test/constraint verification |
| L2687 | `cmd_context()` | CLI handler for budgeted task context compilation |
| L2699 | `cmd_explain()` | CLI handler for one-symbol relationship explanation |
| L2728 | `cmd_why()` | CLI handler for Git-backed symbol history |
| L2759 | `cmd_rollback()` | CLI handler for safe map rollback |
| L2786 | `capture_mcp_command()` | Preserves CLI exit status and output for native MCP tools |
| L2797 | `build_mcp_server()` | Factory initializing FastMCP or MCPServer instance |
| L2813 | `update_map()` | MCP Tool: updates PROJECT_MAP.md via stdio/SSE |
| L2826 | `check_drift()` | MCP Tool: fast MD5 drift check |
| L2842 | `analyze_code_impact()` | MCP Tool: blast radius impact analysis |
| L2855 | `plan_change()` | MCP Tool: graph-backed pre-flight plan and risk score |
| L2860 | `verify_change()` | MCP Tool: change/test/constraint consistency gate |
| L2868 | `compile_context()` | MCP Tool: budgeted task context compiler |
| L2876 | `explain_symbol()` | MCP Tool: unambiguous Stable Symbol explanation |
| L2881 | `explain_symbol_history()` | MCP Tool: Git temporal memory and constraints |
| L2889 | `register_feature()` | MCP Tool: feature traceability registration |
| L2913 | `register_constraint()` | MCP Tool: implicit constraint registration |
| L2930 | `get_map_summary()` | MCP Tool: returns token-saving minified summary |
| L2946 | `cmd_mcp()` | CLI handler for launching MCP server over Stdio or SSE |
